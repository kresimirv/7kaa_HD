...
\usepackage[paperwidth=5.51in, paperheight=8.51in, top=0.6205in, bottom=1.005in, right=0.505in, left=0.705in,]{geometry} % From the original manual. Do not change.
...
\graphicspath{{assets/}}
\usepackage[document]{ragged2e} % load soon to avoid conflicts
\usepackage{pdfpages}
...
% Table of contents
\usepackage{tocloft}
\renewcommand{\cftsecdotsep}{\cftnodots} % remove dots in toc
\renewcommand{\cftsecleader}{\hspace{1em}} % reduce space between sec. and pg # in toc
\renewcommand{\cftchapleader}{\hspace{1em}} % reduce space between chap. and pg # in toc
\renewcommand{\cftchapnumwidth}{0pt} % don't indent chapter titles in toc
\renewcommand\numberline[1]{} % don't number anything in the toc
\renewcommand{\cftaftertoctitle}{\par\noindent\rule{\linewidth}{0.4pt}} % add line under toc title
\addtolength{\cftbeforetoctitleskip}{3cm} % add more whitespace at top of page for toc
\setlength{\cftaftertoctitleskip}{10pt} % reduce whitespace between toc title and first chap title
\setlength{\cftbeforesecskip}{0.25em} % vertical space between section titles in toc
\setlength{\cftsecindent}{2.5em} % indent sectioning in toc
...
\raggedbottom
\setcounter{secnumdepth}{0} % don't number any of the sectioning
...
\setcounter{page}{0}\includepdf{cover-front}
